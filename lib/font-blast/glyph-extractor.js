...
function extractCharsFromFont(fontSvgText, charNameMap, callback) {
    ...
    for (var glyphCount = 0; glyphCount < glyphs.length; glyphCount++) {
        ...
        //I want to match all characters with their charcode
        if (!iconCode) continue;
        if (iconCode.indexOf('&#') != -1) continue;
        if (!iconCode.length || !pathData ) continue;



        iconCode = iconCode.charCodeAt(0);
        // console.log(glyph.getAttribute('unicode'), iconCode);
    ...
    iconSvg.forEach(function (ic, idx) {
        svgo.optimize(ic.svg, function (result) {
            ...
            iconSvg[idx].path = ' fill="#ffff00" '+result.data.match(/d="(.*?)"/)[1];
